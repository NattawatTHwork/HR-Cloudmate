getSessionToken()
    .then(mySession => {
        if (mySession.token && mySession.role === 'admin') {
            fetch(apiUrl + 'application/jobs/get_job_approval_all.php?statusflag=3&language=' + mySession.language, {
                method: 'GET',
                headers: {
...
async function displayTables(datas) {
    ...
    for (const data of datas) {
        ...
        html += `<td>${data.position}</td>
                ...
                <td>
                    <a class="btn btn-danger" href="job_approval_view.php?job_code=${data.job_code}">
                        ${texts.view_data}
                    </a>
                </td>`;
        html += '</tr>';
